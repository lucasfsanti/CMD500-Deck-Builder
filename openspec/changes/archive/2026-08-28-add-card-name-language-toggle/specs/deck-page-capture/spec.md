...
- **THEN** the extension's internal state contains one entry per distinct card per zone with correct English name, Portuguese name, quantity, and lowest price
...
#### Scenario: Card's canonical English name cannot be resolved from the page markup
- **WHEN** a card entry's link markup does not yield a canonical English name (for example, an unrecognized href format)
- **THEN** the extension falls back to the page's own display text for both the card's English name and its Portuguese name, rather than leaving either undefined
